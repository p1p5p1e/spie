<div>
                ...
                <div class="grid md:grid-cols-2 md:gap-6">
                    <div class="relative z-0 mb-6 w-full group">
                        <label for="objective_id"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">{{ __('objective') }}</label>
                        <select id="objective_id" wire:model="objective_id"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option value="">{{ __('Select an option') }}</option>
                            @foreach ($objectives as $objective)
                                <option value="{{ $objective->id }}">{{ $objective->code }}</option>
                            @endforeach
                        </select>
                        <x-jet-input-error for="objective_id" />
                    </div>
                </div>
    ...
    <x-table-form>
        ...
        @endslot
        @slot('paginate')
            {{ $aims->links('vendor.livewire.custom') }}
        @endslot
    </x-table-form>
