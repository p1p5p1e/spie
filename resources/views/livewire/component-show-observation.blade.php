<div>
    ...
    <x-table-form>
                    ...
                    <tbody>
                        @foreach ($observations as $observation)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="py-4 px-6">
                                    @if ($observation->is_complete == false)
                                        {{ __('Pending') }}
                                    @else
                                        {{ __('Complete') }}
                                    @endif
                                ...
                                <td class="py-4 px-6 text-right">
                                    <ul>
                                        @if ($observation->is_complete == false)
                                            <li>
                                                <a wire:click='modalComplete({{ $observation->id }})'
                                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline cursor-pointer">{{ __('To complete') }}</a>
                                            </li>
                                        @endif
                                    </ul>
                                </td>
        ...
        @endslot
        @slot('paginate')
            {{ $observations->links('vendor.livewire.custom') }}
        @endslot
    </x-table-form>

    <x-dialog-modal wire:model="completeModal">
        <x-slot name="title">
        ...
        <x-slot name="content">
            <div class="flex col-span-6 sm:col-span-4 items-center gap-2">
                ...
                <div class="relative z-0 mb-6 w-full group">
                    <select id="is_complete" wire:model="is_complete"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option value="">{{ __('Select an option') }}</option>
                        <option value="1">{{ __('Complete') }}</option>
                        <option value="0">{{ __('Pending') }}</option>
                    </select>
                    <x-jet-input-error for="is_complete" />
                </div>
            </div>
        </x-slot>

        <x-slot name="footer">
            <x-jet-danger-button wire:click="$set('completeModal', false)" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-danger-button>
            <x-jet-secondary-button class="ml-2" wire:click='complete' wire:loading.attr="disabled">
                {{ __('Accept') }}
