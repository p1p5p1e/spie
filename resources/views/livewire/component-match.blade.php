        ...
        @slot('form')
            <form class="mt-2">
                <div class="grid md:grid-cols-4 md:gap-6">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        <label for="indicator_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">
                            {{ __('Indicator') }} PDES
                        </label>

                        <select id="indicator_id" wire:model="indicator_id"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option value="">{{ __('Select an option') }}</option>
                            @foreach ($indicators as $indicator)
                                @if ($indicator->planning)
                                    <option value="{{ $indicator->id }}">{{ $indicator->planning->code }} -
                                        {{ $indicator->description }}</option>
                                @endif
                            @endforeach
                        </select>

                        <x-jet-input-error for="indicator_id" />
                    </div>
                    ...
                    @if ($searchIndicator != null)
                        ...
                        <div class="col-span-1 md:col-span-2">
                            <ul
                                ...
                                @foreach ($indicators as $indicator)
                                    @if ($indicator->planning)
                                        <li wire:click='selectIndicator({{ $indicator->id }})'
                                            class="w-full px-4 py-2 border-b border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-500 cursor-pointer">
                                            {{ $indicator->planning->code }} - {{ $indicator->description }}
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="py-4 px-6">
                                    <ul class="max-w-md space-y-1 text-gray-500 list-disc list-inside dark:text-gray-400">
                                        @foreach ($relation->indicators as $indicator)
                                            <li>
